<?php
	...
	function showPageNumber($i, $c=0, $pr = NULL, $set=false) {
		...
		if ($i != $current) echo '<li><a href="' . site_url("runs/problem/" . $problem->code . "/" . $i) . '">' . $i . '</a></li> ';
		...
		<div id="main">
			<div id="main-container">
				<div id="page-tabs-container">
					<?php
						problem_tabs($is_logged_in, $current_user, TAB_PROBLEM_RUNS, $url, $problem);
					?>
				</div>
				<div class="clear"></div>
				<div id="page-content">
								...
								<table id="problem-runs-list-table" width="100%" class="list">
									...
									<tbody>
										...
										<tr class="run_row" id="run_<?=$run->id?>">
											...
											<a class="edit_link" id="edit_<?=$run->id?>" href="<?=site_url("sections/submit/" . $url . "/" . $run->problem_id . "/" . $run->id)?>"><img src="<?=site_url("assets/theme_new/icons/code_edit.png")?>" /></a>
											<?php } ?></td>
											<?php if ($is_logged_in && $current_user->perm_judge) {	?>
