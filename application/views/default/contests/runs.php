...
<script>
	function show_ce(run_id) {
		...
		<div class="bar">Contest <?=clean4print($contest->label)?> :: My Runs :: Page#<?=$current_page?></div>
		...
		<div class="container">
				...
				<div class="pages">
					...
					<?php
						function showPageNumber($i, $c=0, $set=false) {
							...
							if ($i != $current) echo '<a href="' . site_url("contests/runs/" . $contest->id . "/" . $i) . '">[' . $i . ']</a> ';
			...
			<div class="run listitem">
				<div class="base problem_id2"><a href="<?= site_url("contests/problem/" . $contest->id . "/" . $contest_problem_letters[$run->problem_id])?>"><center><?=$contest_problem_letters[$run->problem_id]?></center></a></div>
			...
			<div class="listitem_ce" id="ce_<?= $run->id ?>"><?= nl2br(clean4print($run->info)) ?></div>
			<? } ?>
