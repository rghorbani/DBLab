		...
		<div id="main">
			<div id="main-container">
				<div id="page-tabs-container">
					<?php
						section_tabs($is_logged_in, $current_user, TAB_SECTION_ADD_PROBLEM);
					?>
				</div>
				<div class="clear"></div>
				<div id="page-content">
					...
					<div id="add_problem" class="shadowed">
						<div class="inner-box">
							<div class="content">
								<?php
									if ($phase == 3) {
										echo '<div class="center error">The problem ' . set_value('code') . ' from ' . $url . 'added successfully</div>';
									}
									function sv($n, $def) {
								...
								?>
								<? if($phase == 1) { ?>
									<form id="add-problem-form" action="<?=site_url("sections/add_existing_problem")?>" method="post">
										<ul class="col1">
											<li><label for="section_from">From:</label></li>

											<li>
												<select id="section_from" name="section_from">
											...
											<li><input id="code" type="text" size="40" name="code" placeholder="Problem Code" /></li>
											<?=form_error('code','<li class="error">','</li>')?>
										</ul>
										
										<ul class="col2">
											<li><label for="section_to">To:</label></li>

											<li>
												<select id="section_to" name="section_to">
												<?php foreach ($sections as $section) {?>
													<option value="<?=$section->id?>" <?=set_select('section_id',$section->id)?>>&nbsp;&nbsp;<?=$section->label?>&nbsp;&nbsp;&nbsp;</option>
												<?php } ?>
												</select>
								...
								<? } else { ?>
									<form id="add-problem-form" action="<?=site_url($CURRENT_PAGE)?>" method="post" enctype="multipart/form-data">
										<ul class="col1">
											<li><label for="code"> Problem Code:</label></li>
											<li><input id="code" type="text" size="40" name="code" value="<?=sv('code', $problem->code)?>" /></li>
											<?=form_error('code','<li class="error">','</li>')?>
										...
										<ul class="col2">
											...
											<li>
												...
												<?php foreach ($sections as $section) {?>
													<option value="<?=$section->id?>" <?=set_select('section_id',$section_id)?>>&nbsp;&nbsp;<?=$section->label?>&nbsp;&nbsp;&nbsp;</option>
												<?php } ?>
												</select>
											</li>
										</ul>
